...
function compose(...args) {
    let funcs = [...args].reverse()
    const len = funcs.length
    let count = -1
    
    function callFunction(newVal) {
      count++
      if(count >= len - 1) return funcs[count](newVal)
      else return callFunction(funcs[count](newVal))
    }
    
    return funcs.length ? val => callFunction(val) : val => val
  }